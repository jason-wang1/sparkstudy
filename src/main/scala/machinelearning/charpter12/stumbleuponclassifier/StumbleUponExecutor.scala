package com.machinelearning.charpter12.stumbleuponclassifier
...
object StumbleUponExecutor {
  ...
  def main(args: Array[String]) {
    ...
    // get dataframe
    val df = spark.read.format("com.databricks.spark.csv").option("delimiter", "\t").option("header", "true")
      .option("inferSchema", "true").load("C:\\Users\\BoWANG\\IdeaProjects\\sparkstudy\\src\\main\\scala\\data\\train.tsv")

    // pre-processing
    df.registerTempTable("StumbleUpon")
    df.printSchema()
    spark.sql("SELECT * FROM StumbleUpon WHERE alchemy_category = '?'").show()
    ...
    df4.printSchema()
    spark.sql("SELECT * FROM StumbleUpon_PreProc").show()
