...
object charpter08 extends App {
  ...
  class CheckingAcount(initialBalance: Double) extends BankAccount(initialBalance) {
    override def deposit(amount: Double): Double = {balance += amount; balance -= 1; balance}

    override def withdraw(amount: Double): Double = {balance -= amount; balance -= 1; balance}
  }
}
