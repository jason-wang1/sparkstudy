...
object decision_trees_2x {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //1 训练样本准备
    val data = spark.read.format("libsvm").load("C:\\Users\\BoWANG\\IdeaProjects\\sparkstudy\\src\\main\\scala\\data\\sample_libsvm_data.txt")
    data.show(false)
    ...
    //11 模型保存与加载
    model1.write.overwrite().save("C:\\Users\\BoWANG\\IdeaProjects\\sparkstudy\\src\\main\\scala\\model\\mlv2")
    val load_treeModel = PipelineModel.load("C:\\Users\\BoWANG\\IdeaProjects\\sparkstudy\\src\\main\\scala\\model\\mlv2")
