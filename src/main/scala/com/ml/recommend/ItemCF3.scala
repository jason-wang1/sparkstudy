...
/**
  ...
  * 而是直接对(userId, itemId, score)评分表进行笛卡尔积。
...
object ItemCF3 {
  def main(args: Array[String]): Unit = {
    ...
    val item2itemDF: DataFrame = df.alias("t1").crossJoin(df.alias("t2"))
      .where($"t1.userId" === $"t2.userId" && $"t1.itemId" < $"t2.itemId")
      .selectExpr("t1.itemId as itemId1", "t2.itemId as itemId2", "t1.score * t2.score as squ")
      .groupBy($"itemId1", $"itemId2")
      .agg(sum($"squ").alias("squareSum"))
      .cache()
    ...
    val itemSimDF: DataFrame = item2itemDF
      ...
      .withColumn("dot", $"squareSum" / ($"item1.vecLen" * $"item2.vecLen"))
